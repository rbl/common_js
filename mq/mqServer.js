...
MQServer.prototype.start = function()
{
  ...
  var self = this;
  this.server.on("connection", function(stream) {self.serverConnection(stream)});
  this.server.on("close",      function() {self.serverClose()});
  this.server.on("error",      function(err) {self.serverError(err)});
...
MQServer.prototype.registerConnection = function(token, ident, connection)
{
  ...
  this.config.registry.register(token, ident, connection, function(err) 
  {
  ...
  var self = this;
  connection.on("close", function() { self.connectionClosed(ident,connection); });
